...
public class LSFWriter
{
    ...
    public void Write(Resource resource)
    {
        ...
        using (this.ValueWriter = new BinaryWriter(ValueStream))
        {
            ...
            Span<byte> stringsCompressed = BinUtils.Compress(stringBuffer, Compression, CompressionLevel);
            Span<byte> nodesCompressed = BinUtils.Compress(nodeBuffer, Compression, CompressionLevel, chunked);
            Span<byte> attributesCompressed = BinUtils.Compress(attributeBuffer, Compression, CompressionLevel, chunked);
            Span<byte> valuesCompressed = BinUtils.Compress(valueBuffer, Compression, CompressionLevel, chunked);
