...
public static class BinUtils
{
    ...
    public static byte[] Decompress(byte[] compressed, int decompressedSize, byte compressionFlags, bool chunked = false)
    {
        switch ((CompressionMethod)(compressionFlags & 0x0F))
        {
            ...
            case CompressionMethod.LZ4:
                if (chunked)
                {
                    var decompressed = LZ4Wrapper.LZ4FrameDecompress(compressed);
                    return decompressed;
                }
                else
                {
                    var decompressed = new byte[decompressedSize];
                    LZ4Codec.Decode(compressed, 0, compressed.Length, decompressed, 0, decompressedSize);
                    return decompressed;
                }
    ...
    public static Span<byte> Compress(byte[] uncompressed, byte compressionFlags)
    {
        return Compress(uncompressed, (CompressionMethod)(compressionFlags & 0x0F), CompressionFlagsToLevel(compressionFlags));
    }

    public static Span<byte> Compress(byte[] uncompressed, CompressionMethod method, CompressionLevel compressionLevel, bool chunked = false)
    {
    ...
    public static Span<byte> CompressZlib(byte[] uncompressed, CompressionLevel compressionLevel)
    {
        int level = zlibConst.Z_DEFAULT_COMPRESSION;
        switch (compressionLevel)
        {
            ...
            case CompressionLevel.DefaultCompression:
                level = zlibConst.Z_DEFAULT_COMPRESSION;
                break;
    ...
    public static Span<byte> CompressLZ4(byte[] uncompressed, CompressionLevel compressionLevel, bool chunked = false)
    {
        if (chunked)
        {
            return LZ4Wrapper.LZ4FrameCompress(uncompressed);
        }
        else
        {
            LZ4Level level = compressionLevel switch
            {
                CompressionLevel.FastCompression => LZ4Level.L00_FAST,
                CompressionLevel.DefaultCompression => LZ4Level.L00_FAST,
                CompressionLevel.MaxCompression => LZ4Level.L12_MAX,
                _ => throw new ArgumentException("Invalid compression level specified")
            };

            return LZ4Wrapper.LZ4BlockCompress(uncompressed, level);
        }
    }
}
